...
const watchDetail: React.FC<watchProps> = ({ watchNews }) => {
  ...
  const [formData, setFormData] = useState({
    collection: `youtube-news`,
    permalink: `${process.env.NEXT_PUBLIC_API_URL}/watch-news/${watchNews?.idx}`,
    idx: watchNews?.idx,
  ...
  const fetchYoutubeData = async () => {
    try {
      const response = await fetch(`/api/comments?collection=youtube-news&idx=${watchNews?.idx}`);
      const commentResponse = await response.json();
      setYoutubeData(Array.isArray(commentResponse) ? commentResponse : [commentResponse]);
    } catch (error) {
